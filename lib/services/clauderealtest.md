...
# Root Cause Analysis & Production-Ready Fix

## The Three Bugs — Explained

### Bug 1: Impossible Upload Speeds (2736 / 3900 Mbps)
**Root Cause:** `StreamedRequest.sink.add(chunk)` does **not** send data over the network. It writes to an **in-memory buffer**. Your code then counts these buffered bytes as "sent":
```dart
// OLD — BROKEN: measures RAM write speed, not network speed
uploadRequest.sink.add(chunkPayload);     // ← instant, writes to RAM buffer
totalBytesSent += chunkPayload.length;    // ← counted before any network I/O
await Future.delayed(Duration(ms: 2));    // ← yields but provides NO backpressure
```
~2700–3900 Mbps is the speed of writing to a Dart `StreamController` buffer in memory.

### Bug 2: Upload Ignores 15-Second Limit
**Root Cause:** Because `sink.add()` is non-blocking, the `while` loop runs at memory speed and dumps hundreds of MB into the buffer in milliseconds. When the timer fires and calls `sink.close()`, the `http.Client` still has to *flush the entire buffer* over the actual network — which can take minutes.

### Bug 3: Gauge Jitter
**Root Cause:** `DateTime.now()` has limited precision (often 1–10ms granularity on mobile). Combined with the `100ms` minimum-interval gate and the EMA weights, instantaneous speed readings swing wildly between ticks.

---

## The Fix

**Key architectural change for upload:** Switch to `dart:io`'s `HttpClient`, which exposes `request.flush()` — a `Future` that **blocks until the OS TCP socket buffer accepts the data**. This is real network backpressure. Bytes are only counted *after* `flush()` succeeds.
...
class RealSpeedHttpService {
  ...
  Future<double> _download(StreamController<RealSpeedResult> ctrl) async {
    ctrl.add(RealSpeedResult(downloadSpeedMbps: 0, status: 'Connecting…'));
    ...
    try { await done.future; } catch (_) { /* force-close may fire error */ }
    ...
    _logger.log('[DL] ${_fmtBytes(totalBytes)} in '
  ...
  Future<double> _upload(
    ...
    kill.cancel();
    ui.cancel();
    try { await request.close(); } catch (_) {}   // best-effort graceful close
    _ulClient = null;

    final mbps = _mbps(totalBytes, sw.elapsedMicroseconds);
    _logger.log('[UL] ${_fmtBytes(totalBytes)} in '
        '${(sw.elapsedMicroseconds / 1e6).toStringAsFixed(1)} s '
        '→ ${mbps.toStringAsFixed(2)} Mbps');
    return mbps;
  }
...
|---|---|---|---|
| **Upload measures RAM speed** | `sink.add()` + count immediately | `request.add()` + `await request.flush()` + count **after** | `flush()` blocks until the OS TCP buffer accepts the data — true network backpressure |
| **Upload ignores 15 s limit** | `sink.close()` still has to drain buffer | `httpClient.close(force: true)` in a `Timer` | Force-close kills all sockets instantly, discarding any unsent buffer |
| **Download ignores 15 s limit** | Timer just sets a flag | `client.close()` in a `Timer` destroys the TCP connection | The response stream gets an error → `Completer` resolves → measurement stops |
| **Impossible Mbps values** | Counted buffered bytes ÷ real time | Only count bytes confirmed by `flush()` ÷ `Stopwatch` time | You can't count faster-than-network bytes anymore |
| **Imprecise timing** | `DateTime.now()` (ms granularity) | `Stopwatch` (µs granularity) | `Stopwatch` uses monotonic high-resolution clock |
| **Gauge jitter** | EMA with 0.4/0.6 weights on 500 ms interval | 1-second **sliding window** average, emitted every 250 ms | A single fast/slow chunk can't spike the EMA; the window smooths naturally |
| **Wrong math risk** | `(bytes * 8 * 1000) / (ms * 1000000)` | `(bytes * 8.0) / µs` | Simpler, avoids intermediate overflow, verified: 1 MB / 1 s = 8.0 Mbps ✓ |
| **Memory: 500 MB upload buffer** | Loop adds to sink at RAM speed → hundreds of MB queued | 512 KB `Uint8List` allocated once, reused | `flush()` paces writes to network speed; only ~512 KB in flight at a time |
| **Compressible upload data** | `chunkPayload[i] = i % 255` (repeating, compressible) | `chunk[i] = (i * 131 + 17) & 0xFF` (LCG pseudo-random) | Prevents ISP transparent compression from inflating results |
| **Cancel doesn't kill upload** | `_isCancelled` flag checked lazily | `cancel()` calls `httpClient.close(force: true)` | Immediately destroys sockets; `flush()` throws → loop exits |